...
while {true} do 
	...
	if(alive player and isciv and player distance Militarybase <= 160 and !("terrorist" call INV_HasLicense))then{titleText ["This is a restricted area! Turn back or you will be shot!", "plain"]};
	if(alive player and isciv and captive player and player distance Militarybase <= 100 and !(player call ISSE_IsVictim))then{format['%1 setcaptive false', player] call broadcast};
	if(alive player and isciv and !firingcaptive and !(captive player) and player distance Militarybase >  100 and !("terrorist" call INV_HasLicense))then{format['%1 setcaptive true', player] call broadcast};
	if(captive player and "terrorist" call INV_HasLicense and !(player call ISSE_IsVictim))then{format['%1 setcaptive false', player] call broadcast};
	if(player call ISSE_IsVictim and !(captive player))then{format['%1 setcaptive true', player] call broadcast};
		
	_veh = vehicle player;
	
	if (player distance getmarkerpos "policebase" < 250 and isciv and !(_veh isKindOf "Air") and !(_veh isKindOf "Ship")) then 
...
while {true} do 
	...
	_bodies = nearestobjects[getpos deadcamlogic,terroristarray, 20];

	if(count _bodies > 0)then{{if(!isPlayer _x)then{deletevehicle _x};} foreach _bodies};

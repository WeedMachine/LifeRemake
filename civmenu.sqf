...
if(typeof player in terroristarray) then
	...
	empty%1 = this;
	this setvehiclevarname ""empty%1"";
	";
	processinitcommands;
	...
	processinitcommands;
	old%1 = objnull;
	empty%1 setpos getpos deadcamlogic;
	[empty%1] join (group server);
	empty%1 removeEventHandler ["Handledamage", 0];
	', rolestring];

	};

waituntil{!(typeof player in terroristarray)};
